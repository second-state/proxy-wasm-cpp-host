...
template <> WasmEdge_Value makeVal<proxy_wasm::Word>(proxy_wasm::Word t) {
...
template <> struct ConvertWordType<proxy_wasm::Word> {
...
template <>
enum WasmEdge_ValType convArgToValType<proxy_wasm::Word>() {
...
template <> proxy_wasm::Word convValTypeToArg<proxy_wasm::Word>(WasmEdge_Value val) {
...
constexpr T convValTypesToArgsTupleImpl(const WasmEdge_Value *arr, std::index_sequence<I...>) {
...
template <typename R, typename T> WasmEdge_FunctionTypeContext *newWasmEdgeFuncType() {
  std::vector<enum WasmEdge_ValType> params, returns;
...
bool WasmEdge::load(std::string_view bytecode, std::string_view,
...
bool WasmEdge::link(std::string_view debug_name) {
  ...
  if (auto num = WasmEdge_StoreListMemoryLength(store_.get()); num > 0) {
    WasmEdge_String name;
    WasmEdge_StoreListMemory(store_.get(), &name, 1);
    memory_ = WasmEdge_StoreFindMemory(store_.get(), name);
  }
  if (auto num = WasmEdge_StoreListFunctionLength(store_.get()); num > 0) {
    std::vector<WasmEdge_String> names(num);
...
void WasmEdge::getModuleFunctionImpl(std::string_view function_name,
  ...
  std::vector<enum WasmEdge_ValType> exp_args, exp_returns;
  convArgsTupleToValTypes<std::tuple<Args...>>(exp_args);
  convArgsTupleToValTypes<std::tuple<>>(exp_returns);
  auto *functype_cxt = WasmEdge_FunctionInstanceGetFunctionType(func_cxt);
